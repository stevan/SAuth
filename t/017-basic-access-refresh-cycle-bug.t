...
foreach ( 0 .. 5 ) {
    ...
    my $extra_access_grant = $provider->process_access_request(
        uid       => $uid,
        hmac      => $access_request->hmac,
        timestamp => $access_request->timestamp,
        body      => $access_request->body->to_json
    );
    isa_ok($extra_access_grant, 'SAuth::Core::AccessGrant');
}
...
my $refreshed_access_grant;
is(exception {
    $refreshed_access_grant = $provider->process_access_refresh(
        uid       => $uid,
        hmac      => $refresh_request->hmac,
        timestamp => $refresh_request->timestamp,
        body      => $refresh_request->body->to_json
    );
}, undef, '... we did not get an exception while refreshing');
