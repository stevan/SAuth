...
BEGIN {
    ...
    use_ok('SAuth::Provider::KeyStore::MongoDB');
...
my $provider = SAuth::Provider->new(
    key_store    => SAuth::Provider::KeyStore::MongoDB->new( collection => $db->get_collection('keys') ),
    token_store  => SAuth::Provider::TokenStore::MongoDB->new( collection => $db->get_collection('access_grants') ),
    capabilities => [qw[
        create
        read
        update
